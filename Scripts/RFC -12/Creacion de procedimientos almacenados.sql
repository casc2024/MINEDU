...
/****** Object:  StoredProcedure [dbo].[USP_ESCALAFON_INS_SERVIDOR_PUBLICO_DATOS_ADICIONALES_IOXML]    Script Date: 25/05/2022 19:14:42 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[USP_ESCALAFON_INS_SERVIDOR_PUBLICO_DATOS_ADICIONALES_IOXML]
@REQUEST_XML XML 	
AS
BEGIN
	...
	IF @Severity = 16 AND @Success = 0
		BEGIN
			...
			VALUES
			('USP_ESCALAFON_INS_SERVIDOR_PUBLICO_DATOS_ADICIONALES_IOXML', @REQUEST_XML, @MessageReal, @Message, @FECHA_ACTUAL, @USUARIO)
		END
...
/****** Object:  StoredProcedure [dbo].[USP_ESCALAFON_SEL_SEPARACION_PREVENTIVAS_OXML]    Script Date: 26/05/2022 10:47:51 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[USP_ESCALAFON_SEL_SEPARACION_PREVENTIVAS_OXML] 
	@ID_SERVIDOR_PUBLICO INT,    
    @pPaginaActual INT,
	@pTamanioPagina INT
AS
BEGIN
	...
	DECLARE @xml_acciones XML = 
		(SELECT		r.Anular, r.MotivoAccion, r.Regimen,r.TipoResolucion, r.TipoResolucionAbreviado, r.NumeroResolucion, r.FechaResolucion, 
					...
					r.Anotaciones, r.Estado, r.IdAccionPorServidorPublicoDatosAdiciones, r.IdAccionPorServidorPublico, r.DescripcionResultado,
		 ...
		 FROM	(	
						SELECT	CONVERT(BIT,CASE WHEN er.CODIGO_ESTADO_REGISTRO IN (5) THEN 1 Else 0  END) AS Anular
								...
								, aspda.DESCRIPCION_RESULTADO AS DescripcionResultado
...
/****** Object:  StoredProcedure [dbo].[USP_ESCALAFON_INS_DATOS_SANCION_JUDICIAL]    Script Date: 27/05/2022 14:12:08 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
'<root>
  <datos_sancion CAUSA_SANCION="fg" FECHA_NOTIFICACION="2022-05-26T05:00:00Z" FECHA_INICIO="2022-05-27T05:00:00Z" FECHA_FIN="2022-06-01T05:00:00Z" DIAS_SANCION="5" ID_SERVIDOR_PUBLICO="163184" ID_TIPO_RESOLUCION="21" NUMERO_RESOLUCION="234234-2342-34234" FECHA_RESOLUCION="2022-04-26T05:00:00Z" ENTIDAD_EMISORA_RESOLUCION="sdf" NOMBRE_DOCUMENTO="archivoPrueba.pdf" NOMBRE_ARCHIVO="2/785c8262-12de-ec11-b81b-0050569005a4" FOLIOS_DOCUMENTO="2" ES_DESTITUCION="false" OBSERVACION="dsf" CANTIDAD_ANIOS_INHABILITACION="0" FECHA_INICIO_INHABILITACION="1900-01-01T00:00:00" FECHA_FIN_INHABILITACION="1900-01-01T00:00:00" FECHA_CESE="1900-01-01T00:00:00" />
  <usuario>
    <row USUARIO="42813053" NOMBRE_USUARIO="CHRISTIAN ABELARDO SCHEREIBER CORNELIO" ID_CENTRO_AMBITO="267" ID_TIPO_CENTRO_AMBITO="2" AMBITO="DRE Lima Metropolitana" CODIGO_ROL="GSD002" ROL="Técnico de escalafón " FECHA_CREACION="27/05/2022 06:13 p. m." ID_USUARIO="80874" />
  </usuario>
</root>'
...
/****** Object:  StoredProcedure [dbo].[USP_ESCALAFON_INS_DATOS_SANCION_ADMINISTRATIVA]    Script Date: 27/05/2022 21:28:16 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
ALTER PROCEDURE [dbo].[USP_ESCALAFON_INS_DATOS_SANCION_ADMINISTRATIVA] 
@REQUEST_XML XML      
AS      
BEGIN      
...
--exec [dbo].[USP_ESCALAFON_MOD_SERVIDOR_PUBLICO_DATOS_ADICIONALES_IOXML]
--'
--<root>
--  <accion ID_SERVIDOR_PUBLICO="5648875" FECHA_FIN="2022-05-01T05:00:00Z" ID_RESULTADO="1" DESCRIPCION_RESULTADO="Absuelto" ID_SEPARACION_PREVENTIVA="-1" ID_ACCION_POR_SERVIDOR_PUBLICO_DATOS_ADICIONALES="29" />
--  <usuario>
--    <row USUARIO="42813053" NOMBRE_USUARIO="CHRISTIAN ABELARDO SCHEREIBER CORNELIO" ID_CENTRO_AMBITO="267" ID_TIPO_CENTRO_AMBITO="2" AMBITO="DRE Lima Metropolitana" CODIGO_ROL="GSD002" ROL="Técnico de escalafón " FECHA_CREACION="26/05/2022 12:17 a. m." ID_USUARIO="80874" />
--  </usuario>
--</root>
--'

CREATE PROCEDURE [dbo].[USP_ESCALAFON_MOD_SERVIDOR_PUBLICO_DATOS_ADICIONALES_IOXML]
@REQUEST_XML XML 	
AS
BEGIN
	...
	DECLARE		
		...
		@ID_RESULTADO INT,		
		@DESCRIPCION_RESULTADO VARCHAR(250),
		@ID_ACCION_POR_SERVIDOR_PUBLICO_DATOS_ADICIONALES INT,		
		@ID_SEPARACION_PREVENTIVA INT,		
		@ACCION VARCHAR(100),
		@USUARIO VARCHAR(20),
		@NOMBRE_USUARIO VARCHAR(50),
	...
	BEGIN TRY
		...
		SELECT					
				...
				@DESCRIPCION_RESULTADO = UPPER(c.value('@DESCRIPCION_RESULTADO', 'VARCHAR(250)')),
				@ID_SEPARACION_PREVENTIVA = c.value('@ID_SEPARACION_PREVENTIVA', 'INT'),
				@ID_ACCION_POR_SERVIDOR_PUBLICO_DATOS_ADICIONALES = c.value('@ID_ACCION_POR_SERVIDOR_PUBLICO_DATOS_ADICIONALES', 'INT')				
		FROM	@REQUEST_XML.nodes('/root/accion') x(c);	
		...
		IF (@ID_ACCION_POR_SERVIDOR_PUBLICO_DATOS_ADICIONALES != -1) 
		BEGIN
			UPDATE  [transaccional].[accion_por_servidor_publico_datos_adicionales] 
			SET 
				FECHA_FIN = @FECHA_FIN,
				ID_RESULTADO = @ID_RESULTADO,
				DESCRIPCION_RESULTADO = @DESCRIPCION_RESULTADO
			WHERE ID_ACCION_POR_SERVIDOR_PUBLICO_DATOS_ADICIONALES = @ID_ACCION_POR_SERVIDOR_PUBLICO_DATOS_ADICIONALES		
		END
	...
	IF @Severity = 16 AND @Success = 0
		BEGIN
			...
			([NOMBRE_SP], [INPUT_PARAMS_XML], [MENSAJE_BD], [MENSAJE_USUARIO], [FECHA_CREACION], [USUARIO_CREACION])
			VALUES
			('USP_ESCALAFON_MOD_SERVIDOR_PUBLICO_DATOS_ADICIONALES_IOXML', @REQUEST_XML, @MessageReal, @Message, @FECHA_ACTUAL, @USUARIO)
		END
...
GO
/****** Object:  StoredProcedure [dbo].[USP_ESCALAFON_JOB_FINALIZA_ACCIONES]    Script Date: 26/05/2022 15:36:34 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
SELECT  ID_LOG_FINALIZA_ACCION, ID_ACCION_POR_SERVIDOR_PUBLICO, ID_SERVIDOR_PUBLICO, INDICADOR, TRAMA_DETALLE, MENSAJE
FROM	auditoria.log_finaliza_accion_detalle
WHERE   ID_LOG_FINALIZA_ACCION = 15


INDICADOR:
0: Error
1 y 3: No actualizó situación laboral
2 y 4: Actualizó situación laboral
5: Solo actualizó la vigencia de la ultima accion

EXEC [dbo].[USP_ESCALAFON_JOB_FINALIZA_ACCIONES]

*/
ALTER PROCEDURE [dbo].[USP_ESCALAFON_JOB_FINALIZA_ACCIONES]
AS
BEGIN
	...
	DECLARE @FECHA_ACTUAL DATETIME = GETDATE(),
			@FECHA_FUTURO DATETIME = DATEADD(YEAR,100,GETDATE()),
			@ID_NUEVO_SITACION_LABORAL INT,
			@ID_SERVIDOR_PUBLICO INT,
			@ID_ACCION_POR_SERVIDOR_PUBLICO INT,
			@FECHA_INICIO_ULTIMA_ACCION DATETIME,
			@FECHA_FIN DATETIME,
			@ID_CONDICION_LABORAL_ACTUAL INT,
			@ID_SITUACION_LABORAL_ACTUAL INT,
			@ID_ASP_ULTIMA_ACCION INT,
			@ULTIMA_ACCION VARCHAR(250),
			@ES_VIGENTE BIT,
			@HOY DATE,
			@AYER DATE,
			@ID_SANCION_POR_SERVIDOR_PUBLICO INT,
			@SITUACION_LABORAL VARCHAR(250),
			...
			@ID_LOG_FINALIZA_ACCION INT,
			@TT_ACCIONES INT = 0,
			@CON INT = 1,
			@SW INT = 0
		
		SET @HOY = @FECHA_ACTUAL
		SET @AYER = DATEADD(DAY,-1, @HOY)

		DECLARE @tmpACCIONES_POR_FINALIZAR TABLE 
		...
		INSERT INTO @tmpACCIONES_POR_FINALIZAR
		(ID_ACCION_POR_SERVIDOR_PUBLICO, ES_VIGENTE, ID_MOTIVO_ACCION, FECHA_INICIO, FECHA_FIN, ID_SERVIDOR_PUBLICO, ID_SITUACION_LABORAL)
		SELECT	--TOP 5 
				ASP.ID_ACCION_POR_SERVIDOR_PUBLICO, 
				ASP.ES_VIGENTE,
				ASP.ID_MOTIVO_ACCION, 
				CONVERT(DATE,ASP.FECHA_INICIO_ACCION_POR_SERVIDOR_PUBLICO), 
				CONVERT(DATE,ASP.FECHA_FIN_ACCION_POR_SERVIDOR_PUBLICO), 
				ASP.ID_SERVIDOR_PUBLICO, ISNULL(SLFMA.ID_SITUACION_LABORAL,0)
		FROM	transaccional.accion_por_servidor_publico AS ASP WITH (NOLOCK)
				INNER JOIN parametrica.situacion_laboral_fin_motivo_accion AS SLFMA WITH (NOLOCK) 
					ON ASP.ID_MOTIVO_ACCION = SLFMA.ID_MOTIVO_ACCION
				INNER JOIN sistema.estado_registro AS ER WITH (NOLOCK) 
					ON ASP.ID_ESTADO_REGISTRO = ER.ID_ESTADO_REGISTRO
				INNER JOIN transaccional.accion_por_servidor_publico_registro AS ASPR WITH (NOLOCK)
					ON ASP.ID_SERVIDOR_PUBLICO = ASPR.ID_SERVIDOR_PUBLICO
				INNER JOIN sistema.situacion_laboral AS SL WITH (NOLOCK)
					ON ASPR.ID_SITUACION_LABORAL = SL.ID_SITUACION_LABORAL
		WHERE	SLFMA.ES_VIGENTE = 1
				AND ASP.FECHA_FIN_ACCION_POR_SERVIDOR_PUBLICO < @FECHA_ACTUAL
				AND YEAR(ASP.FECHA_FIN_ACCION_POR_SERVIDOR_PUBLICO) >= YEAR(@FECHA_ACTUAL) - 1
				AND ISNULL(ASP.ACCION_FINALIZADA,0) = 0
				AND ER.CODIGO_ESTADO_REGISTRO IN (5,13,19,20,23) --APROBADO,EN TRASLADO,PROGRAMADO,APROBADO CON SOLICITUD DE ANULACIÓN,LIMITADO
				--AND ASP.ID_ACCION_POR_SERVIDOR_PUBLICO IN (53,150,160,319,5623308,677,721,705,746)
				AND SL.CODIGO_ORIGEN <> 15 -- DIFERENTE DE BAJA
		ORDER BY ID_SERVIDOR_PUBLICO, FECHA_FIN_ACCION_POR_SERVIDOR_PUBLICO
		SET @TT_ACCIONES = @@ROWCOUNT

		INSERT INTO [auditoria].[log_finaliza_accion]
		([FECHA_INICIA], [TOTAL_REGISTROS])
		VALUES
		(@FECHA_ACTUAL, @TT_ACCIONES)
		SET @ID_LOG_FINALIZA_ACCION = IDENT_CURRENT('auditoria.log_finaliza_accion');

		WHILE @CON <= @TT_ACCIONES
			BEGIN

				BEGIN TRY	

					BEGIN TRANSACTION PROCESO;  

					SET @SW = 0
					SET @ID_NUEVO_SITACION_LABORAL = 0
					SET @Success = 0
					SET @ES_VIGENTE = 0

					SELECT	@ID_NUEVO_SITACION_LABORAL = ID_SITUACION_LABORAL,
							@ID_SERVIDOR_PUBLICO = ID_SERVIDOR_PUBLICO,
							@ID_ACCION_POR_SERVIDOR_PUBLICO = ID_ACCION_POR_SERVIDOR_PUBLICO,
							@FECHA_FIN = FECHA_FIN,
							@ES_VIGENTE = ES_VIGENTE
					FROM	@tmpACCIONES_POR_FINALIZAR
					WHERE	ID = @CON					

					DECLARE @xml_log_inicio AS XML = 
						(	SELECT	@ID_ACCION_POR_SERVIDOR_PUBLICO AS ID_ACCION_POR_SERVIDOR_PUBLICO,
									@ID_SERVIDOR_PUBLICO AS ID_SERVIDOR_PUBLICO,
									...
									USUARIO_MODIFICA
							FROM	transaccional.accion_por_servidor_publico_registro AS r WITH (NOLOCK)
							WHERE	r.ID_SERVIDOR_PUBLICO = @ID_SERVIDOR_PUBLICO
							FOR XML AUTO ,TYPE
						)					
  
					SELECT	@ID_CONDICION_LABORAL_ACTUAL = ID_CONDICION_LABORAL,
							@ID_SITUACION_LABORAL_ACTUAL = ID_SITUACION_LABORAL
					FROM	transaccional.accion_por_servidor_publico_registro AS ASPR WITH (NOLOCK)
					WHERE	ASPR.ID_SERVIDOR_PUBLICO = @ID_SERVIDOR_PUBLICO

					IF @ID_NUEVO_SITACION_LABORAL <> 0 --ACTUALIZANDO LA SITUACION LABORAL DEL SERVIDOR PUBLICO
						BEGIN
							SET @SW = 1

							IF @ID_SITUACION_LABORAL_ACTUAL <> @ID_NUEVO_SITACION_LABORAL
								BEGIN
									IF EXISTS(	SELECT	SL.ID_SITUACION_LABORAL
												FROM	sistema.situacion_laboral AS SL WITH (NOLOCK)
												WHERE	SL.ID_SITUACION_LABORAL = @ID_NUEVO_SITACION_LABORAL
														AND SL.ID_CONDICION_LABORAL = @ID_CONDICION_LABORAL_ACTUAL)
										BEGIN											

											--DETERMINAR LA ULTIMA ACCION VIGENTE
											SELECT	TOP 1 @FECHA_INICIO_ULTIMA_ACCION = CONVERT(DATE,ASP.FECHA_INICIO_ACCION_POR_SERVIDOR_PUBLICO)
											FROM	transaccional.accion_por_servidor_publico AS ASP WITH (NOLOCK)
													INNER JOIN sistema.motivo_accion AS MA WITH (NOLOCK) ON ASP.ID_MOTIVO_ACCION = MA.ID_MOTIVO_ACCION
													INNER JOIN sistema.situacion_laboral_motivo_accion AS SLMA WITH (NOLOCK) 
														ON ASP.ID_MOTIVO_ACCION = SLMA.ID_MOTIVO_ACCION
													INNER JOIN sistema.estado_registro AS ER WITH (NOLOCK)
														ON ASP.ID_ESTADO_REGISTRO = ER.ID_ESTADO_REGISTRO
											WHERE	ASP.ID_SERVIDOR_PUBLICO = @ID_SERVIDOR_PUBLICO
													AND ER.CODIGO_ESTADO_REGISTRO IN (5,13,19,20)
													AND @FECHA_ACTUAL BETWEEN 
														CONVERT(DATE,FECHA_INICIO_ACCION_POR_SERVIDOR_PUBLICO) AND 
														CONVERT(DATE,ISNULL(FECHA_FIN_ACCION_POR_SERVIDOR_PUBLICO, @FECHA_FUTURO))
													AND ASP.ID_ACCION_POR_SERVIDOR_PUBLICO <> @ID_ACCION_POR_SERVIDOR_PUBLICO
													AND SLMA.AFECTA_SITUACION_LABORAL = 1
													AND SLMA.ES_VIGENTE = 1
											ORDER BY CONVERT(DATE,FECHA_INICIO_ACCION_POR_SERVIDOR_PUBLICO) DESC, 
													ID_ACCION_POR_SERVIDOR_PUBLICO DESC

											IF @@ROWCOUNT > 0 AND @FECHA_INICIO_ULTIMA_ACCION < @FECHA_FIN 
												BEGIN
													SET @SW = 2

													UPDATE	ASPR
													SET		ID_SITUACION_LABORAL = CJ.ID_SITUACION_LABORAL,
															SITUACION_LABORAL = CJ.DESCRIPCION_SITUACION_LABORAL,
															ID_CONDICION_LABORAL = CJ.ID_CONDICION_LABORAL,
															CONDICION_LABORAL = CJ.DESCRIPCION_CONDICION_LABORAL,
															ES_EXTRABAJADOR = CJ.ES_CESE,
															FECHA_MODIFICA = @FECHA_ACTUAL,
															USUARIO_MODIFICA = 'JOB SISTEMA'
													FROM	transaccional.accion_por_servidor_publico_registro AS ASPR WITH (NOLOCK)
															CROSS JOIN
															(
																SELECT	SL.ID_SITUACION_LABORAL, 
																		SL.DESCRIPCION_SITUACION_LABORAL, 
																		SL.ID_CONDICION_LABORAL, 
																		CL.DESCRIPCION_CONDICION_LABORAL,
																		CONVERT(BIT,CASE WHEN SL.CODIGO_ORIGEN = -1 THEN 1 ELSE 0 END) ES_CESE
																FROM	sistema.situacion_laboral AS SL WITH (NOLOCK)
																		INNER JOIN sistema.condicion_laboral AS CL WITH (NOLOCK) ON SL.ID_CONDICION_LABORAL = CL.ID_CONDICION_LABORAL
																WHERE	SL.ID_SITUACION_LABORAL = @ID_NUEVO_SITACION_LABORAL
															) AS CJ
													WHERE	ASPR.ID_SERVIDOR_PUBLICO = @ID_SERVIDOR_PUBLICO

													UPDATE	SP
													SET		ES_EXTRABAJADOR = ASPR.ES_EXTRABAJADOR										
													FROM	maestro.servidor_publico AS SP WITH (NOLOCK)
															INNER JOIN transaccional.accion_por_servidor_publico_registro AS ASPR WITH (NOLOCK)
																ON SP.ID_SERVIDOR_PUBLICO = ASPR.ID_SERVIDOR_PUBLICO
												END
										END
								END
						END
					ELSE
						BEGIN
							SET @SW = 3
							--DETERMINAR LA ULTIMA ACCION VIGENTE
							SELECT	TOP 1 @ID_NUEVO_SITACION_LABORAL = SLMA.ID_SITUACION_LABORAL,
									@ID_ASP_ULTIMA_ACCION = ASP.ID_ACCION_POR_SERVIDOR_PUBLICO,
									@ULTIMA_ACCION = MA.DESCRIPCION_MOTIVO_ACCION,
									@FECHA_INICIO_ULTIMA_ACCION = CONVERT(DATE,ASP.FECHA_INICIO_ACCION_POR_SERVIDOR_PUBLICO)
							FROM	transaccional.accion_por_servidor_publico AS ASP WITH (NOLOCK)
									INNER JOIN sistema.motivo_accion AS MA WITH (NOLOCK) ON ASP.ID_MOTIVO_ACCION = MA.ID_MOTIVO_ACCION
									INNER JOIN sistema.situacion_laboral_motivo_accion AS SLMA WITH (NOLOCK) 
										ON ASP.ID_MOTIVO_ACCION = SLMA.ID_MOTIVO_ACCION
							...
							IF @@ROWCOUNT > 0 AND @FECHA_INICIO_ULTIMA_ACCION < @FECHA_FIN 
								BEGIN									
									...
									IF @ID_SITUACION_LABORAL_ACTUAL <> @ID_NUEVO_SITACION_LABORAL
										BEGIN
											...
											SET		ID_SITUACION_LABORAL = CJ.ID_SITUACION_LABORAL,
													...
													ID_ACCION_POR_SERVIDOR_PUBLICO = @ID_ASP_ULTIMA_ACCION,
													FECHA_MODIFICA = @FECHA_ACTUAL,
													ES_EXTRABAJADOR = CJ.ES_CESE,
													USUARIO_MODIFICA = 'JOB SISTEMA'
											FROM	transaccional.accion_por_servidor_publico_registro AS ASPR WITH (NOLOCK)
									...
									IF @ES_VIGENTE = 1 --SI LA ACCION QUE TERMINA ES LA VIGENTE
										BEGIN
											SET @SW = 5

											UPDATE	transaccional.accion_por_servidor_publico
											SET		ES_VIGENTE = 0, FECHA_MODIFICACION = @FECHA_ACTUAL, USUARIO_MODIFICACION = 'JOB SISTEMA'
											WHERE	ID_ACCION_POR_SERVIDOR_PUBLICO = @ID_ACCION_POR_SERVIDOR_PUBLICO

											UPDATE	transaccional.accion_por_servidor_publico
											SET		ES_VIGENTE = 1, FECHA_MODIFICACION = @FECHA_ACTUAL, USUARIO_MODIFICACION = 'JOB SISTEMA'
											WHERE	ID_ACCION_POR_SERVIDOR_PUBLICO = @ID_ASP_ULTIMA_ACCION

											UPDATE	ASPR
											SET		ACCION = @ULTIMA_ACCION,
													ID_ACCION_POR_SERVIDOR_PUBLICO = @ID_ASP_ULTIMA_ACCION,
													FECHA_MODIFICA = @FECHA_ACTUAL,
													USUARIO_MODIFICA = 'JOB SISTEMA'
											FROM	transaccional.accion_por_servidor_publico_registro AS ASPR WITH (NOLOCK)
					...
					UPDATE	transaccional.accion_por_servidor_publico
					SET		ACCION_FINALIZADA = 1
					WHERE	ID_ACCION_POR_SERVIDOR_PUBLICO = @ID_ACCION_POR_SERVIDOR_PUBLICO
	
					DECLARE @xml_log_final AS XML = 
						(	SELECT	@SW AS INDICADOR,
									@ID_NUEVO_SITACION_LABORAL AS ID_NUEVO_SITACION_LABORAL,
									ID_SITUACION_LABORAL,
									SITUACION_LABORAL,
									ID_CONDICION_LABORAL,
									CONDICION_LABORAL,
					...
					DECLARE @xml_log AS XML = (
							SELECT  @xml_log_inicio AS 'SITUACION_INICIAL',
									@xml_log_final AS 'SITUACION_FINAL'
							FOR     XML PATH('') ,ROOT('ROOT')
						);

					INSERT INTO [auditoria].[log_finaliza_accion_detalle]
					([ID_LOG_FINALIZA_ACCION], [FECHA_PROCESAMIENTO], [ID_ACCION_POR_SERVIDOR_PUBLICO], [ID_SERVIDOR_PUBLICO], [INDICADOR], [TRAMA_DETALLE])
					VALUES
					(@ID_LOG_FINALIZA_ACCION, GETDATE(), @ID_ACCION_POR_SERVIDOR_PUBLICO, @ID_SERVIDOR_PUBLICO, @SW, @xml_log)
					SET @ID_LOG_FINALIZA_ACCION = IDENT_CURRENT('auditoria.log_finaliza_accion');

					SET @Success = 1

					COMMIT TRANSACTION PROCESO;
				...
				BEGIN CATCH  
					...
					IF @@TRANCOUNT > 0  
						ROLLBACK TRANSACTION PROCESO;

				END CATCH;

				IF @Success = 0
					BEGIN
						DECLARE @xml_log_error AS XML = (
								SELECT  @xml_log_inicio AS 'SITUACION_INICIAL'
								FOR     XML PATH('') ,ROOT('ROOT')
							);

						INSERT INTO [auditoria].[log_finaliza_accion_detalle]
						([ID_LOG_FINALIZA_ACCION], [FECHA_PROCESAMIENTO], [ID_ACCION_POR_SERVIDOR_PUBLICO], [ID_SERVIDOR_PUBLICO], [INDICADOR], [TRAMA_DETALLE], [MENSAJE])
						VALUES
						(@ID_LOG_FINALIZA_ACCION, GETDATE(), @ID_ACCION_POR_SERVIDOR_PUBLICO, @ID_SERVIDOR_PUBLICO, 0, @xml_log_error, @Message)
						SET @ID_LOG_FINALIZA_ACCION = IDENT_CURRENT('auditoria.log_finaliza_accion');
					END
		...
		--SANCIONES ADMINISTRATIVA, CAMBIA ESTADO DE "CON SANCION" A "EN ACTIVIDAD"
		SET @CON = 1
		SET @TT_ACCIONES = 0
		SELECT	ROW_NUMBER() OVER(ORDER BY S.ID_SANCION_POR_SERVIDOR_PUBLICO) AS ID,
				ID_SERVIDOR_PUBLICO,
				ID_SANCION_POR_SERVIDOR_PUBLICO
		INTO	#SERVIDORES
		FROM	transaccional.sancion_por_servidor_publico AS S WITH (NOLOCK)
				INNER JOIN sistema.tipo_sancion AS TS WITH (NOLOCK)
					ON S.ID_TIPO_SANCION = TS.ID_TIPO_SANCION
				INNER JOIN sistema.estado_registro AS ER WITH (NOLOCK)
					ON S.ID_ESTADO_REGISTRO_SANCION = ER.ID_ESTADO_REGISTRO
		WHERE	S.FECHA_FIN_SANCION_SANCION_POR_SERVIDOR_PUBLICO = @AYER
				AND TS.CODIGO_TIPO_SANCION IN (4,6,11,12)
		...
		IF @CON <= @TT_ACCIONES
			BEGIN				
				...
				SELECT	@ID_SERVIDOR_PUBLICO = ID_SERVIDOR_PUBLICO,
						@ID_SANCION_POR_SERVIDOR_PUBLICO = ID_SANCION_POR_SERVIDOR_PUBLICO
				FROM	#SERVIDORES
				WHERE	ID = @CON

				--SITUACION INICIAL
				DECLARE @xml_log_inicia_sancion AS XML = 
					(	SELECT	@ID_SANCION_POR_SERVIDOR_PUBLICO AS ID_SANCION_POR_SERVIDOR_PUBLICO,
								@ID_SERVIDOR_PUBLICO AS ID_SERVIDOR_PUBLICO,
								ID_SITUACION_LABORAL,
								SITUACION_LABORAL,
								ID_CONDICION_LABORAL,
				...
				--OBTENIENDO LA CONDICION LABORAL ACTUAL
				SELECT	@ID_CONDICION_LABORAL_ACTUAL = ID_CONDICION_LABORAL
				FROM	transaccional.accion_por_servidor_publico_registro AS R WITH (NOLOCK)
				WHERE	R.ID_SERVIDOR_PUBLICO = @ID_SERVIDOR_PUBLICO

				--DETERMINANDO EL ID_SITUACION_LABORAL
				SELECT	@ID_NUEVO_SITACION_LABORAL = ID_SITUACION_LABORAL,
						@SITUACION_LABORAL = DESCRIPCION_SITUACION_LABORAL
				FROM	sistema.situacion_laboral AS SL WITH (NOLOCK)
				WHERE	DESCRIPCION_SITUACION_LABORAL = 'EN ACTIVIDAD'
						AND CODIGO_SITUACION_LABORAL IN (1, 11)
						AND ES_VIGENTE = 1
						AND ID_CONDICION_LABORAL = @ID_CONDICION_LABORAL_ACTUAL

				--ACTUALIZANDO LA SITUACION LABORAL ACTUAL
				UPDATE	transaccional.accion_por_servidor_publico_registro
				SET		ID_SITUACION_LABORAL = @ID_NUEVO_SITACION_LABORAL,
						SITUACION_LABORAL = @SITUACION_LABORAL,
						FECHA_MODIFICA = @FECHA_ACTUAL,
						USUARIO_MODIFICA = 'JOB SISTEMA'
				WHERE	ID_SERVIDOR_PUBLICO = @ID_SERVIDOR_PUBLICO
				...
				--SITUACION FINAL
				DECLARE @xml_log_final_sancion AS XML = 
				...
				DECLARE @xml_log_sancion AS XML = (
						SELECT  @xml_log_inicia_sancion AS 'SITUACION_INICIAL',
								@xml_log_final_sancion AS 'SITUACION_FINAL'
						FOR     XML PATH('') ,ROOT('ROOT')
					);
				...
				([ID_LOG_FINALIZA_ACCION], [FECHA_PROCESAMIENTO], [ID_ACCION_POR_SERVIDOR_PUBLICO], [ID_SERVIDOR_PUBLICO], [INDICADOR], [TRAMA_DETALLE])
				VALUES
				(@ID_LOG_FINALIZA_ACCION, GETDATE(), @ID_SANCION_POR_SERVIDOR_PUBLICO, @ID_SERVIDOR_PUBLICO, 1, @xml_log_sancion)
				SET @ID_LOG_FINALIZA_ACCION = IDENT_CURRENT('auditoria.log_finaliza_accion');

				SET @CON = @CON + 1
			END

		--SANCIONES JUDICIALES, CAMBIA ESTADO DE "CON SANCION" O "CESE" A "EN ACTIVIDAD"
		SET @CON = 1
		SET @TT_ACCIONES = 0
		SELECT	ROW_NUMBER() OVER(ORDER BY S.ID_SANCION_POR_SERVIDOR_PUBLICO) AS ID,
				ID_SERVIDOR_PUBLICO,
				ID_SANCION_POR_SERVIDOR_PUBLICO
		INTO	#SERVIDORES_SANCION_JUDICIAL
		FROM	transaccional.sancion_por_servidor_publico AS S WITH (NOLOCK)
				INNER JOIN sistema.tipo_sancion AS TS WITH (NOLOCK)
					ON S.ID_TIPO_SANCION = TS.ID_TIPO_SANCION
				INNER JOIN sistema.estado_registro AS ER WITH (NOLOCK)
					ON S.ID_ESTADO_REGISTRO_JUDICIAL = ER.ID_ESTADO_REGISTRO
		WHERE	(S.FECHA_FIN_INHABILITACION = @AYER 
				OR S.FECHA_FIN_SANCION_SANCION_POR_SERVIDOR_PUBLICO = @AYER)
				AND TS.CODIGO_TIPO_SANCION IN (9)
				AND ER.CODIGO_ESTADO_REGISTRO = 5 --APROBADO
		SET @TT_ACCIONES = @@ROWCOUNT
		...
		IF @CON <= @TT_ACCIONES
			BEGIN		
				SELECT	@ID_SERVIDOR_PUBLICO = ID_SERVIDOR_PUBLICO,
						@ID_SANCION_POR_SERVIDOR_PUBLICO = ID_SANCION_POR_SERVIDOR_PUBLICO
				FROM	#SERVIDORES_SANCION_JUDICIAL
				WHERE	ID = @CON
			...
			END
		UPDATE	[auditoria].[log_finaliza_accion]
		SET		FECHA_FINALIZA = GETDATE()
		WHERE	ID_LOG_FINALIZA_ACCION = @ID_LOG_FINALIZA_ACCION
END
